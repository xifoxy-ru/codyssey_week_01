...
run_step "4단계: [git -C \"$REPO_ROOT\" rev-parse --is-inside-work-tree] Git 저장소 여부 확인" \
  'git -C "$REPO_ROOT" rev-parse --is-inside-work-tree'

run_step "5단계: [git -C \"$REPO_ROOT\" branch --show-current] 현재 브랜치 확인" \
  'git -C "$REPO_ROOT" branch --show-current'

run_step "6단계: [git -C \"$REPO_ROOT\" remote -v] 원격 저장소 연결 확인" \
  'git -C "$REPO_ROOT" remote -v || true'

run_step "7단계: [git -C \"$REPO_ROOT\" status --short] 작업 트리 상태 확인" \
  'git -C "$REPO_ROOT" status --short'

run_step "8단계: [git -C \"$REPO_ROOT\" log --oneline -n 3] 최근 커밋 확인" \
  'git -C "$REPO_ROOT" log --oneline -n 3 || true'

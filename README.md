...
프로젝트 디렉토리는 **실습 주제별 분리**, **재현 가능성**, **로그 추적 용이성**을 기준으로 구성하였다.

- `00_cli`, `01_permission`, `02_docker`, `03_github` 는 과제의 핵심 실습 단위를 기준으로 분리하였다.
- 각 디렉토리에는 해당 단계만 실행할 수 있는 `run_*.sh` 스크립트를 두어, 특정 단계만 개별적으로 재실행할 수 있도록 구성하였다.
- Docker 파트는 기본 점검, 커스텀 웹 실행, 볼륨 테스트처럼 성격이 다른 실습이 섞여 있으므로 `02_docker` 아래에 세부 스크립트(`00_run_docker_check.sh`, `01_run_custom_web.sh`, `02_run_volume_test.sh`)로 다시 분리하였다.
- 각 단계별 결과는 해당 디렉토리 안의 로그 파일에 남기도록 하여, 어떤 명령이 어떤 결과를 만들었는지 추적하기 쉽게 하였다.
- Bonus 항목은 필수 과제와 구분되도록 `04_bonus_compose`, `05_bonus_github` 로 별도 분리하였다.

즉, 이 구조는 단순히 파일을 나열한 것이 아니라, **실습 단위별 독립 실행**, **로그 관리**, **검수 시 확인 용이성**을 기준으로 설계한 것이다.
...
이번 과제에서는 포트 매핑과 볼륨 테스트를 단순히 수동으로 한 번 실행하는 것이 아니라, **같은 명령을 다시 실행해도 같은 결과를 얻을 수 있도록 스크립트 기반으로 정리**하였다.

- 포트 매핑 실습은 `01_run_custom_web.sh` 안에서 이미지 빌드 → 컨테이너 실행 → `curl` 확인 순서로 고정하였다.
- 같은 이름의 컨테이너가 이미 존재할 수 있으므로, 실행 전에 `docker rm -f ... >/dev/null 2>&1` 형태로 기존 테스트 컨테이너를 정리하도록 구성하였다.
- 바인드 마운트 실습은 호스트의 `site/index.html` 을 수정하고, 같은 주소에 다시 `curl` 하여 변경 사항이 즉시 반영되는지 확인하는 방식으로 재현 가능하게 만들었다.
- 볼륨 테스트는 항상 `docker volume create` → 첫 번째 컨테이너에서 데이터 저장 → 컨테이너 삭제 → 두 번째 컨테이너에서 같은 데이터 확인 순서로 고정하였다.
...
즉, 재현 가능성은 단순히 명령을 기록한 것이 아니라, **사전 정리 → 실행 → 확인 → 로그 기록**의 흐름을 스크립트로 고정함으로써 확보하였다.
...
### 8-11. 이미지와 컨테이너의 차이: 빌드 / 실행 / 변경 관점

Docker에서 이미지와 컨테이너는 서로 다른 역할을 가진다.

#### 1. 빌드 관점
이미지는 Dockerfile을 바탕으로 만들어지는 **설계 결과물**이다.  
예를 들어 이번 과제에서는 `Dockerfile` 에서 `nginx:alpine` 을 기반으로 하고, `site/` 디렉토리를 복사하여 `codyssey-custom-web:1.0` 이미지를 빌드하였다.

즉, 이미지는 **실행 전 미리 만들어 두는 템플릿**에 가깝다.

#### 2. 실행 관점
컨테이너는 이미지를 바탕으로 실제로 실행되는 **동작 중인 인스턴스**이다.  
같은 이미지 하나로도 여러 컨테이너를 만들 수 있으며, 각 컨테이너는 독립적으로 실행된다.

예를 들어:
- `codyssey-custom-web:1.0` 이미지를 이용해 `codyssey-web-8080` 컨테이너를 실행할 수 있다.
- 같은 시점에 별도의 설정으로 다른 컨테이너를 또 실행할 수도 있다.

즉, 이미지는 원본 템플릿이고, 컨테이너는 그 템플릿을 기반으로 실제 실행한 결과이다.

#### 3. 변경 관점
이미지 자체는 빌드가 끝나면 내용이 고정된다. 따라서 호스트 파일을 수정해도, 이미 이미지 안으로 복사된 파일은 자동으로 바뀌지 않는다.  
반면 컨테이너는 실행 중에 파일이 바뀔 수 있고, 특히 바인드 마운트를 사용하면 호스트 파일 수정이 즉시 반영될 수 있다.

이번 과제에서는 이 차이를 다음과 같이 확인하였다.

- `COPY site/ /usr/share/nginx/html/` 로 만든 이미지는 빌드 시점의 파일 내용을 가진다.
- `-v "$WEB_DIR/site:/usr/share/nginx/html"` 로 실행한 바인드 마운트 컨테이너는 호스트 파일과 직접 연결되므로, 호스트 파일 수정이 즉시 반영된다.

즉,
- **이미지**는 빌드 시점의 내용을 가진 고정된 결과물이고
- **컨테이너**는 이미지를 바탕으로 실행되며
- **변경 반영 방식**은 단순 이미지 실행인지, 바인드 마운트인지에 따라 달라진다.
...
절대 경로와 상대 경로는 상황에 따라 다르게 선택하는 것이 좋다.

- **절대 경로**는 기준 위치가 바뀌어도 항상 같은 위치를 가리켜야 할 때 적합하다.
  - 예: 시스템 설정 파일, 사용자 홈 디렉토리, 고정된 저장 위치를 참조할 때
  - 예: `/Users/사용자이름/...`, `$HOME/.ssh/id_ed25519`

- **상대 경로**는 현재 프로젝트 내부에서 함께 이동하는 파일 구조를 다룰 때 적합하다.
  - 예: 현재 디렉토리 기준으로 실행하는 스크립트, 프로젝트 내부 리소스 참조
  - 예: `./00_cli/run_cli.sh`, `./web`, `./img/vscode.png`

실무적으로는 다음 기준으로 선택할 수 있다.

1. **프로젝트 내부에서 함께 관리되는 파일**이면 상대 경로가 더 이식성이 좋다.
2. **실행 위치가 달라져도 반드시 같은 위치를 가리켜야 하는 자원**이면 절대 경로가 더 안전하다.
3. 스크립트에서는 실행 위치에 따라 경로가 흔들리는 문제를 줄이기 위해 `BASE="$(cd "$(dirname "$0")" && pwd -P)"` 로 기준 디렉토리를 먼저 고정한 뒤, 그 아래에서는 상대적인 하위 경로를 조합하는 방식이 가장 실용적이었다.

즉, 이번 과제에서는 **기준 디렉토리는 절대적으로 고정하고, 그 아래 리소스는 상대적으로 조합하는 방식**을 주로 사용하였다.

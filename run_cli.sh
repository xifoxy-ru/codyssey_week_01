...
step "2단계: [mkdir answer_directory] 폴더 생성"
echo "=== [mkdir answer_directory] 결과 ===" >> "$LOG"
...
step "4단계: [touch test] 빈 파일 생성"
echo "=== [touch test] 결과 ===" >> "$LOG"
touch "$BASE/test"
ls -la "$BASE/test" >> "$LOG"
ls -la "$BASE/test"
gap

step "5단계: [cp test test_copy] 복사"
...
step "6단계: [mv test_copy test_renamed] 이름 변경 혹은 이동"
...
step "7단계: [rm test_renamed test] 삭제"
echo "=== [rm test_renamed test] 결과 ===" >> "$LOG"
rm "$BASE/test_renamed" "$BASE/test"
ls -la "$BASE/" >> "$LOG"
ls -la "$BASE/"
gap

step "8단계: [cat terminal_cli] 파일 표준출력 "
